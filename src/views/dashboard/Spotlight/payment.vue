                ...
                <div>
                  ...
                  >
                    <b-form-radio
                      ...
                      value="full"
                      checked
                      >Pay $ {{ total_price }} in full today</b-form-radio
                    >
                  </b-form-group>
              ...
              <div class="">
                ...
                <div class="">
                  ...
                  <div class="subtotal-section">
                    ...
                    <!-- <div class="subtotal-section-row">
                      <b-input-group prepend="Voucher" class="mt-3">
                        <b-form-input v-model="voucher"></b-form-input>
                        <b-input-group-append>
                          <b-button variant="primary" @click="apply_coupen"
                            >Apply</b-button
                          >
                        </b-input-group-append>
                      </b-input-group>
...
export default {
  ...
  methods: {
    ...
    populate_stripe_element() {
      ...
      this.cardElement = this.elements.create("card", { style: this.style }); // Create an instance of the card Element.
      this.cardElement.mount("#stripe-element");
      ...
      axios
        ...
        .then((res) => {
          if (res.data.status == true) {
            let percent = res.data.promo.percent;
            let saleprice =
              this.total_price - this.total_price * (percent / 100);
            this.total_price = Math.round(saleprice);
          }
        });
    },
    recurring() {
      ...
      stripe
        .handleCardSetup(self.intent_secrets.client_secret, self.cardElement, {
          payment_method_data: {
        ...
        .then(function (result) {
          if (result.error) {
            ...
            let package_detail = self.package_detail;
            if (payment_method) {
              self.checkoutLoader = true;
              self.loader = true;
              axios
                .post(
                  `${apiUrl}/subscribion/recurring`,
                  {
    ...
    //   let package_detail = self.package_detail;
    //   stripe.createToken(self.cardElement).then(function (result) {
    //
    //     if (result.error) {
    //       // Inform the user if there was an error.
    //       self.carderror = result.error.message;
    ...
    get_package() {
      ...
      axios
        ...
        .then((res) => {
          ...
          this.total_price = res.data.package_detail.price;
        });
    },
    ...
    loadpaypal() {
      ...
      script.src = `https://www.paypal.com/sdk/js?client-id=${Paypal.production}&currency=USD`;

      //script.src = `https://www.paypal.com/sdk/js?client-id=${Paypal.Live}&vault=false`;
  ...
  updated() {
    this.get_intent_secrets();
    this.initializeAffirm();
    this.loadpaypal();
    this.populate_stripe_element();
    //document.getElementById("stripe-element").innerHTML = "<p>html 123</p>";
  },
  async mounted() {
    this.get_package();
