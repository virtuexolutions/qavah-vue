<template>
  <div v-cloak>
    <b-card class="rounded-lg" v-if="check_if_premium_exist('discrete-mode')">
      <h2>Your Profile Is Hidden</h2>
      <b-overlay :show="profileSettingLoader">
        <template #overlay>
          <GridLoader
            class="custom-class"
            color="#93652b"
            :loading="profileSettingLoader"
            :size="10"
            sizeUnit="px"
          />
        </template>
    ...
    <!-- For Free Users -->
    <b-card class="rounded-lg" v-if="currentUser.subscription.length === 0">
      <b-media class="private-mode-card">
        <template #aside>
        ...
        <h5 class="mt-0">Discretion Mode</h5>
        <p class="mb-3">Only can be seen by members you like or manage</p>
        <b-link class="custom-link" @click="openModal('discretionModal')"
          >Choose who sees you
          <svg
            xmlns="http://www.w3.org/2000/svg"
            height="20"
    ...
    >
      ...
      <div class="body">
        <!-- 1 -->
        <div class="info">
          <p class="title">How discretion mode works</p>
          ...
          <p class="subtitle">
            ...
            see different profiles for just $10.00 more monthly.
          </p>
        </div>
        <!-- 2 -->
        <div class="info">
          ...
          <p class="subtitle">
            You can either send a fancy or love note to someone to make your
            profile visible only to them.
          </p>
        </div>
        <!-- 3 -->
        <div class="info">
            ...
            >
              ...
              <g data-name="katman 2" id="katman_2">
                <g id="icons">
                  ...
                  <polygon class="cls-1" points="48 12 24 12 20 8 44 8 48 12" />
                  <polygon
                    class="cls-3"
        ...
        <!-- 4 -->
        <div class="info">
          <p class="title">Monitor who can view you</p>
      ...
      <div style="gap: 20px" class="btn-container">
        <b-link class="custom-btn-1" href="/dashboard/powerups">
          OK, GOT IT!
        </b-link>
        <b-button class="custom-btn-1" @click="closeModal('discretionModal')">
          Not Now
    ...
    >
      ...
      <p class="mt-4">
        Do you want to unhide your profile and send your like ? after all , you
        want them to know you're interested!
      </p>
    ...
    <!-- Choose Users To Show Profile -->
    <!-- Private Mode Modal -->
    <b-modal
      id="profileChooseUsersModal"
      content-class="rounded-lg"
      body-class="rounded-lg"
      centered
      hide-footer
      hide-header
    >
      <h2 class="text-center">Choose Who Can See You</h2>
      ...
      <vue-autosuggest
        class="autosuggest mt-4"
        :input-props="{
          id: 'autosuggest__input',
          class: 'form-control rounded-lg custom-field',
      ...
      <div class="my-2 custom-tag-container">
        <b-badge
          pill
        ...
        >
          {{ user.profileName }}
          <i class="remove-icon" @click="removeUser(index)">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="20"
              viewBox="0 0 20 20"
      ...
      <div class="btn-container-2">
        <b-button
          block
          class="custom-btn-1 mt-4"
          @click="updateProfileSetting('choose')"
          >OK</b-button
        >
        <b-button
          block
          variant="empty"
          @click="closeModal('profileChooseUsersModal')"
          >Not Now</b-button
        >
      </div>
    </b-modal>
      ...
      sectionConfigs: {
        Users: {
          ...
          onSelected: (selected) => {
            ...
            this.users.push(selected.item);
          },
        },
      },
      // Vue Auto Suggest End
      selected: "",
      options: [
  ...
  methods: {
    ...
    fetchResults() {
      ...
      this.timeout = setTimeout(() => {
        const searchPromise = axios.post(`${apiUrl}/user/search-users`, body, {
          headers,
        });
        Promise.all([searchPromise]).then((values) => {
          ...
          const users = values[0].data.data;
    ...
    renderSuggestion(suggestion) {
      ...
      // return suggestion.item.profileName;
      return (
        <div class="d-flex align-items-center" style="cursor: pointer;">
          <b-avatar class="mr-2" src={suggestion.item.profile_images[0].url} />
          <p class="p-0 m-0 text-capitalize">{suggestion.item.profileName}</p>
        </div>
      );
    },
    ...
    hideUser() {
      ...
      axios
        .post(`${apiUrl}/options/hiddenUsers`, data, { headers })
        .then((res) => {
          let data = res.data;

          if (res.status === 200) {
            this.users = data.hide_users;
          } else {
            // Vue.$toast.open({
    ...
    updateProfileSetting(option) {
      ...
      axios
        .post(`${apiUrl}/user/profile-settings`, data, { headers })
        .then((res) => {
          // console.log("res -> ", res);
          const data = res.data;
          if (res.status === 200) {
            ...
            Vue.$toast.open({
              message: `Unknown Error Occurred: ${data.error}`,
              type: "error",
            });
          }
        })
    ...
    chooseWhoSeeYou() {
      ...
      // console.log("data -> ", data);
      axios
        .post(`${apiUrl}/options/chooseWhoSeeYou`, data, { headers })
        .then((res) => {
          // console.log("res -> ", res);
          if (res.status === 200) {
            // Vue.$toast.open({
    ...
    //   setTimeout(() => {
    //     axios
    //       .get(`${apiUrl}/user/get-my-profile?myUid=${this.currentUser.uid}`)
    //       .then((res) => {
    //         // console.log("res -> ", res);
    //         if (res.status === 200) {
    //           const profile = res.data.result;
    //           this.user = profile;
    //           this.selected = profile.profile.type;
    ...
    //   }, 500);
    // },
    discreteModeSubscription(subscription) {
      let sub = subscription;

      if (sub.length > 0) {
        // if(sub.includes('platinum')) {
        // }
        // if(sub.includes('gold')) {
        // }
        // if(sub.includes('gold')) {
        // }
      }
    ...
    onOptionChange() {
      if (this.selected === "public") {
        if (this.currentUser.subscription.length === 0) {
          this.updateProfileSetting(this.selected);
        } else {
          this.updateProfileSetting(this.selected);
        }
      }

      if (this.selected === "private") {
      ...
      if (this.selected === "choose") {
        if (this.currentUser.subscription.length === 0) {
          ...
          if (
            ...
            this.openModal("profileChooseUsersModal");
          } else {
            this.openModal("discretionModal");
          }
        }
